...
- **Current Implementation Landing**:
  ...
  - On **2026-04-15**, the DTQ-04 follow-up ship repaired the branch-local shell/audio compile drift without changing the owning product contract: shell-side compat shims now cover sync-island events, auto-reconnect scheduling, session-preview audio-context derivation, and the SIM audio drawer's relative sync label / last-sync timestamp path. Focused verification passed with `./gradlew --offline :app-core:compileFullDebugKotlin`; the remaining unverified surface is still device-level shell/audio behavior rather than local compile health.
