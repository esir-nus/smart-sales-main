...
| 03 | skills-align | cancelled | Cancelled 2026-04-24 — agent-dir files (`.claude/`, `.codex/`, `.agent/`, `AGENTS.md`) moved to personal/untracked so repo-level skill-doc rewrite scope is moot. See Cross-Sprint Decisions below. | — |
...
- **Sprint 03 cancellation (2026-04-24):** sprint 03 was cancelled, not executed. The user reclassified `.claude/`, `.codex/`, `.agent/`, and `AGENTS.md` as personal/untracked via `git rm --cached` + `.gitignore` additions, so there are no repo-tracked skill docs left for the sprint to rewrite. The Ship cadence decision above is therefore permanent rather than transitional: `/merge` and `/ship` run under the rules each contributor maintains in their local copy, and per-repo governance defers to the sprint-contract spec itself. Any future ship-rule alignment happens at the spec layer (`docs/specs/sprint-contract.md`, `docs/specs/ship-time-checks.md`) rather than through command/skill docs.
