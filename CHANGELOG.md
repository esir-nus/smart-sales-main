...
- **[维护] develop 主分支协作路由收敛** — 统一 feature 与 harmony 分支默认向 `develop` 发起协作与交付，移除平台分支作为日常目标的旧路由，简化 `/merge` 与 `/ship` 治理规则。

### 2026-04-18

- **[新增] 配对完成后的隔离态提示与锁存** — 连接建立后会主动暴露 WiFi isolation 风险并锁存关键上下文，让 ConnectivityModal、配对流程与音频同步链路更早联动修复提示。
- **[修复] 配对后疑似 WiFi isolation 预警** — 徽章完成配对后若检测到疑似 WiFi isolation，系统会更早标记风险并推动修复提示，减少后续同步失败才暴露问题的情况。
...
- **[修复] 鸿蒙音频链路 ArkTS 类型安全问题** — 修复 Phase 2A 音频管线中的 ArkTS 类型约束与兼容性问题，降低 `.ets` 编译失败风险。
- **[修复] 鸿蒙 Tingwu 说话人分离与详情 UI 对齐** — 校正 Tingwu 说话人分离结果在鸿蒙音频详情界面的映射与展示，减少转写细节错位。
- **[修复] 徽章下载前台保活链路恢复** — 补齐下载通知通道与前台服务保活 wiring，避免录音下载过程中前台任务丢失。
- **[修复] 转写音频抽屉卡片细节优化** — 调整音频抽屉卡片的界面表现与交互细节，改善转写内容浏览体验。
- **[维护] 鸿蒙治理与协作约定补齐** — 新增 ArkTS 合规规则与 agent coalition 协作文档，并同步 Claude/Codex 参考入口，降低鸿蒙原生迁移期间的协作偏差。
- **[维护] SmartSales 用户手册补齐** — 新增成体系的 SmartSales 使用手册，便于用户统一查阅快速开始、聊天、日程与连接相关说明。
...
- **[发布] 鸿蒙平台音频处理基础设施** — 为鸿蒙平台发布了音频处理的隔离容器架构。
