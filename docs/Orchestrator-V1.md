...
- **M2B TranscriptionDerivedState** (dynamic, derived within transcription session)
  ...
  - > [!NOTE] **M1 Deferred**: `chapters[]`, `keyPoints[]`, and rich source pointers are not implemented in M0. Current implementation uses `TranscriptMetadata` with basic fields (`speakerMap`, `mainPerson`, `shortSummary`).
- **M3 SessionState**
...
| `publishedPrefixBatchIndex` | integer | Monotonic | Last published batch index (prefix boundary) | TranscriptPublisher | PublisherState, PublishedTranscript | **M1 Deferred** |
