...
- **Current Carry Debt**:
  ...
  - On **2026-04-21**, the scheduler polish lane added a dev-gated text-injection harness for deterministic Path A replay on device: `MainActivity` now accepts scheduler dev extras, the SIM scheduler drawer auto-open target can pin a requested date before transcript injection, and `scripts/scheduler_dev_capture.sh` captures screenshot + logcat + UI dump in one loop. The same pass also stabilized `SchedulerCalendar` month anchoring so selected-date highlight survives parent recomposition, while SIM batch/error copy stays aligned with scheduler-owned strings. Focused evidence: `:app-core:testFullDebugUnitTest --tests com.smartsales.prism.ui.sim.SimSchedulerViewModelTest`, adb captures under `build/scheduler-captures/`; targeted instrumentation remained partially blocked by OEM/device-side process crashes during `connectedFullDebugAndroidTest`.
