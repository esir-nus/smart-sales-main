...
generateHarmonyConfig();

export function generateHarmonyConfig(): void {
  const localProperties = loadLocalProperties(LOCAL_PROPERTIES_PATH);
  const config = resolveRuntimeConfig(localProperties);
  const missing = REQUIRED_KEYS.filter((key: keyof HarmonyRuntimeConfig) => {
    return config[key].trim().length === 0;
  });

  if (missing.length > 0) {
    throw new Error(
      `[HarmonyConfig] Missing required keys in local.properties: ${missing.join(', ')}`
    );
  }
...
export function loadLocalProperties(filePath: string): Record<string, string> {
  if (!fs.existsSync(filePath)) {
    throw new Error(
      `[HarmonyConfig] Missing repo-root local.properties at ${filePath}`
    );
  }
...
function buildGeneratedModule(config: HarmonyRuntimeConfig): string {
  const rows = Object.entries(config).map(([key, value]) => {
    return `  ${key}: '${escapeForSingleQuotedString(value)}'`;
  });

  return `import type { HarmonyRuntimeConfig } from './HarmonyContainerConfig';

// Auto-generated from repo-root local.properties by hvigorfile.ts.
// Do not edit or commit this file.
export const harmonyRuntimeConfig: HarmonyRuntimeConfig = {
${rows.join(',\n')}
};
`;
}
...
function splitPropertyLine(rawLine: string): { key: string; value: string } {
  ...
  return {
    key: rawLine.slice(0, keyEnd),
    value: rawLine.slice(cursor)
  };
}
