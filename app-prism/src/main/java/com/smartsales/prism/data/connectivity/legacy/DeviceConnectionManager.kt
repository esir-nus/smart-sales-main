...
class DefaultDeviceConnectionManager @Inject constructor(
    ...
    private val bleGateway: com.smartsales.prism.data.connectivity.legacy.gateway.GattSessionLifecycle,
    ...
    private fun startNotificationListener(session: BleSession) {
        ...
        notificationListenerJob = scope.launch(dispatchers.io) {
            ...
            bleGateway.listenForBadgeNotifications().collect { event ->
                when (event) {
                    is com.smartsales.prism.data.connectivity.legacy.gateway.BadgeNotification.RecordingReady -> {
                        ConnectivityLogger.i("📥 Badge recording ready: ${event.filename}")
                        _recordingReadyEvents.tryEmit(event.filename)
                    }
                    is com.smartsales.prism.data.connectivity.legacy.gateway.BadgeNotification.TimeSyncRequested -> {
                        ConnectivityLogger.d("⏰ Badge time sync requested")
                    }
                    is com.smartsales.prism.data.connectivity.legacy.gateway.BadgeNotification.Unknown -> {
