...
| HS-006 | Complete-native app — Phase 2B scheduler | Backend/Pipeline | Active | 2026-04-22 |
...
**Scope**:
- `platforms/harmony/smartsales-app/entry/src/main/ets/domain/Scheduler.ets`
- `platforms/harmony/smartsales-app/entry/src/main/ets/pages/Index.ets`
- `platforms/harmony/smartsales-app/entry/src/main/ets/pages/SchedulerPage.ets`
- `platforms/harmony/smartsales-app/entry/src/main/ets/services/FileStore.ets`
- `platforms/harmony/smartsales-app/entry/src/main/ets/services/SchedulerRepository.ets`
- `docs/platforms/harmony/test-signing-ledger.md` (Phase 2B signing entry)
- `docs/plans/harmony-tracker.md` (H-04 Phase 2B Notes/Drift update — allowed as governance companion per declaration-first §5)

**Pipeline contract**:
- Seeded scheduler ingress → domain commit → local persistence → rendered scheduler state inside the complete-native app, without widening reminder-alarm delivery into this sprint.

**Critical dataflow joints**:
1. Seeded scheduler ingress → `Scheduler.ets` domain commit
2. `Scheduler.ets` → `SchedulerRepository.ets` persistence
3. `SchedulerRepository.ets` → `FileStore.ets` local JSON save/load
4. `SchedulerRepository.ets` → `SchedulerPage.ets` rendering

**Mini-lab scenarios**:
- Operator-seeded exact-create execution.
- Operator-seeded vague-create execution.
- Conflict reschedule.
- Reopen persisted schedule without backend rerun.

**Success criteria**:
- `hvigor` build succeeds; `hdc install` succeeds on a signed HarmonyOS device; `hilog` shows each critical dataflow joint firing end to end.
- Evidence stays aligned with `docs/platforms/harmony/test-signing-ledger.md`.

**Deferred scope**:
- Reminder/alarm platform-adapter delivery stays in a future HS-NNN sprint.
...
**Evidence log**:
- 2026-04-22: commit `10ff36d98` — domain + repository + page + test-signing-ledger update on `harmony/scheduler-phase-2b`; pending device hilog.

**Branch Restore Snapshot**:
- Branch: `harmony/scheduler-phase-2b`
- Baseline: `origin/platform/harmony`
- Current head: `10ff36d98`
- Restore confidence: clean worktree verified; type-check + hvigor build pending.
