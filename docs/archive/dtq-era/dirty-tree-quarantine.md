...
> **Purpose**: Active lane/build-state ledger for quarantining, splitting, landing, or parking the current dirty worktree before any branch-promotion step.
> **Status**: Active
...
4. If a lane needs files from another active lane, split the scope first. Do not silently share write scope.
5. `docs/plans/tracker.md` stays the campaign index only. This tracker owns lane-by-lane operational state.
6. No branch-default switch, Android beta baseline tag, or release-branch cut is allowed while this tracker still contains unassigned or mixed dirty work.
7. No lane may move to `Accepted` while its doc-code alignment state is unresolved.
...
| `Active` | Lane is currently in building / cleanup / integration |
...
| `DTQ-06` | Governance, trackers, repo guardrails, and multi-device registry | `.gitignore`, `.github/**`, `docs/specs/platform-governance.md`, `docs/reference/platform-targets.md`, `docs/platforms/**`, `docs/plans/tracker.md`, `docs/plans/sim-tracker.md`, `docs/cerb/interface-map.md`, dirty-tree audit/control-plane docs, `data/connectivity/registry/**`, `ConnectivityModal.kt`, `ConnectivityViewModel.kt`, `SimShellDynamicIslandCoordinator.kt`, `RealPairingService.kt` | feature-owned code lanes except for final docs sync references | `Active` | `Both pending` | compile + focused tests (`*RealPairingServiceTest`, `*ConnectivityViewModelTest`) plus docs sync for multi-device registry layer | — | Reopened 2026-04-15: multi-device registry layer (`DeviceRegistry`, `DeviceRegistryManager`, `SharedPrefsDeviceRegistry`), `ConnectivityModal` complete rewrite for multi-device management, `RealPairingService` registry integration, and `SimShellDynamicIslandCoordinator` activeDeviceName require fresh lane stabilization. Temporary ownership overrides DTQ-03 `data/connectivity/**` glob for `registry/` and DTQ-04 `ui/sim/**` glob for `SimShellDynamicIslandCoordinator`. DTQ-03 must review disputed files before merge. |
...
- `DTQ-06` moved from `Active` to `Accepted` after a local governance pass confirmed the control-plane docs exist, the Harmony compatibility guide is explicitly legacy-scoped, and the current Android tree contains neither native Harmony artifacts nor `ohos.*` imports.
