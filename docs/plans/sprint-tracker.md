...
| HS-005 | Complete-native app — Phase 2A audio pipeline | Backend/Pipeline | Accepted | 2026-04-20 |
| HS-006 | Complete-native app — Phase 2B scheduler | Backend/Pipeline | Active | 2026-04-20 |
...
**Outcome / Drift**:
- Accepted 2026-04-20. Device L3 evidence for the end-to-end audio pipeline (hvigor build → `hdc install` → launch → hilog joints firing) was captured within the HS-010 session on device `4NY0225613001090` (2026-04-18). Service layer (`HttpClient`, `FileStore`, `Picker`, `OssService`, `TingwuService`, `AudioRepository`, `AudioPage`) complete and on-device verified. Domain model translations (`SignatureUtils`, `FormatUtils`, `Audio.ets`, `AppConfig`/`AppRuntimeConfig`) match Kotlin `domain/` semantics. Phase 2B (HS-006) activates next.
...
**Pipeline contract**:
- Create form input → `ScheduledTask` domain construction → `SchedulerRepository.ets` write → `FileStore.ets` persistence (`smartsales_scheduler_tasks.json`).
- Cold-launch restore: `FileStore.loadTasks()` → `SchedulerRepository.ets` in-memory state → `SchedulerPage.ets` list render.
- Mutation (complete/delete) → `SchedulerRepository.ets` update → `FileStore.saveTasks()` → UI refresh.
- Reminder path (conditional): task with future `startTime` → `reminderAgentManager.publishReminder()` → OS-level alarm registration; cancellation on delete.

**Critical dataflow joints**:
1. Create form → `ScheduledTask` construction with all required fields (`id`, `title`, `startTime`, `urgencyLevel`, `isCompleted`, `createdAt`, `updatedAt`) — verify field mapping matches Android `ScheduledTask.kt` semantics.
2. `SchedulerRepository.ets` write → `FileStore.saveTasks()` → JSON persisted under `smartsales_scheduler_tasks.json`; cold-launch restore reads same file → list populated with correct tasks.
3. Complete mutation: `isCompleted = true` → persisted → `SchedulerPage.ets` renders completion state correctly on reload.
4. Delete mutation: task removed from in-memory state + file; app restart confirms absence.
5. (Conditional) Reminder registration: task with future `startTime` → `reminderAgentManager.publishReminder()` call observed in hilog; OS notification fires at scheduled time OR is cancelled on task delete. If `reminderAgentManager` requires permissions or AGC config not yet provisioned, this joint defers to HS-011 — document explicitly in Evidence log.

**Mini-lab scenarios**:
- Create a task (title "客户拜访", date tomorrow 14:00, urgency HIGH) → appears in scheduler list → kill/relaunch app → task still present with correct fields. (Joints 1, 2)
- Mark the above task complete → completion indicator visible → kill/relaunch → persisted as `isCompleted: true`. (Joint 3)
- Delete the task → removed from list → kill/relaunch → absence confirmed. (Joint 4)
- (Joint 5 if reminder path in-scope): create a task with `startTime` 2 minutes in the future → OS notification fires at that time; delete the task → verify notification is cancelled via `reminderAgentManager.cancelReminder()`. If not reproducible in mini-lab, record blocker and defer to HS-011.
...
- hvigor build succeeds; `hdc install` accepted on device `smartsales.HOS.test`; hilog shows joints 1–4 (and 5 if attempted) firing.
...
- `platforms/harmony/smartsales-app/entry/src/main/ets/services/AudioRepository.ets` — in-memory + FileStore persistence pattern to follow exactly (SchedulerRepository mirrors this shape)
- `platforms/harmony/smartsales-app/entry/src/main/ets/services/FileStore.ets` — existing file I/O; add `loadTasks()` / `saveTasks()` alongside existing `loadArtifacts()` / `saveArtifacts()`
- `platforms/harmony/smartsales-app/entry/src/main/ets/domain/Scheduler.ets` — stub domain types already present; complete and verify field parity
- `platforms/harmony/smartsales-app/entry/src/main/ets/pages/SchedulerPage.ets` — stub awaiting implementation
- Android `domain/scheduler/src/main/java/com/smartsales/prism/domain/scheduler/ScheduledTask.kt` — canonical field set to mirror
- Android `domain/scheduler/src/main/java/com/smartsales/prism/domain/scheduler/UrgencyLevel.kt` — enum to mirror (already partially done in `Scheduler.ets`)
