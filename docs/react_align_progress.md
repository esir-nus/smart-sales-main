...
最新进展（2025-11-29）
- Home 垂直三页壳：已切换为 Device/Home/Audio 的三页 pager（无静态留白），anchor 修正中，待 Audio/Device tag 可见性测试通过。
...
- 测试状态：主要仪表测试已通，剩余 Audio/Device 导航、ChatHistory 会话返回等待修复 anchor 与 pagerIndex 重置后重跑。
- React UI audit：继续保持 Home/Sidebar/AudioFiles/UserCenter 已对齐的交互与文案。
- 下一步：完善垂直 pager anchor/offset，确保 PAGE_AUDIO_FILES/PAGE_DEVICE_MANAGER 可见；重跑导航相关仪表；补充 Home/DeviceManager 视觉细节与拖拽动画对齐 React。
