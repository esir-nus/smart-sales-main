...
### 2026-04-21
- **harmony / hs-006-scheduler-phase-2c**: HS-006 Phase 2C implementation landed on the complete-native Harmony lane in `platforms/harmony/smartsales-app`. The Harmony scheduler now adds GUID-preserving reschedule, conflict evaluation during add/reschedule/persist/initialize, deterministic overlap harness seeding, inline reschedule controls, and per-row conflict caution rendering while keeping reminder publish/cancel honestly deferred to the HS-011 stub seam. Docs were synced in `docs/plans/harmony-tracker.md` and `docs/platforms/harmony/test-signing-ledger.md`; Phase 2C still requires the 2026-04-21 on-device L3 pass before it can be called device-verified.

### 2026-04-20
- **harmony / hs-006-scheduler-phase-2b**: HS-006 SHIPPED on the complete-native Harmony lane. Implemented the scheduler create/list/complete/delete slice in `platforms/harmony/smartsales-app` with Android-parity `ScheduledTask` fields, a new in-memory plus persisted `SchedulerRepository`, `FileStore` task load/save support, and a real scheduler page wired into the app shell. On-device verification on `4NY0225613001090` covered build, AGC signing, install, launch, create, cold restore, complete, delete, and final empty restore via screenshots, layout dumps, and `hilog`. The harness surfaced an ArkUI row-reuse bug after completion, and the final shipped fix keys task rows by `id + updatedAt + completion state` so restored completed tasks render correctly.
