...
|----------------|--------------|------------------|-------|-------------|-------|
| `home_shell` | `sim_home_hero_shell.html` | empty home shell chrome + active chat shell chrome | Shipped | Prototype HTML + accepted audit | Shipped shell chrome basis for the SIM family. Logged 2026-04-02: RuntimeShell now owns dynamic-island lane arbitration in production, keeping scheduler as the default island, allowing approved connectivity interrupts/heartbeat in the same center slot, routing tap to the visible lane, and preserving scheduler-only downward drag. Logged 2026-04-02 fidelity follow-up: the dark empty-home header now matches the approved ambient-icon handoff, so the connected lane renders borderless chroma-led island text plus shell-owned decorative left/right ambient flank icons, while inline battery-inside-island rendering is retired. Deferred: real battery sourcing is still mock/viewmodel-backed in this slice. |
