...
| `UX.CONNECTIVITY.MODAL.DEVICE_LIST` | Registered device cards | actionable | stable ordered cards | registry/default/active changes | empty, default first, active selected | `DeviceRegistryManager.registeredDevices`; `DeviceRegistryManager.activeDevice` | switch, rename, set default, remove | default card stays pinned first; non-default cards follow oldest successful-pairing order; active selection changes border/content only and must not reorder cards | `docs/cerb/connectivity-bridge/spec.md` |
