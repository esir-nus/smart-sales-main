...
- [x] **T1.6: Validation**
  ...
  - [x] **2026-03-25 Android Studio SIM APK Entry**: swapped the default `MAIN/LAUNCHER` activity to `SimMainActivity` so standard Android Studio install/run and APK packaging now boot directly into the SIM shell while `AgentMainActivity` remains declared for explicit legacy/internal entry only
...
- [x] **T4.8: Native Banner / Alarm Integration**
  ...
  - [x] **2026-03-21 Focused L1**
    `SimSchedulerViewModelTest` now proves: exact create reminder scheduling, conflict-exact reminder scheduling, vague no-op alarm behavior with reminder prompt emission, mixed `Uni-M` exact/vague batches scheduling only exact reminders while still emitting one create-scope prompt, delete cancellation, mark-done cancellation, restore-without-rearm, reschedule cancel-plus-rearm, and one-shot reminder-reliability prompt emission while scheduling continues. `:app-core:testDebugUnitTest --tests "com.smartsales.prism.ui.sim.SimSchedulerViewModelTest"` and `:app-core:compileDebugKotlin` are green.
...
- [x] **T5.1: Flow / Shell Entry Contract**
  ...
  - [x] **2026-03-20 Execution Brief**
    Wave 5 execution is now compressed in `docs/plans/sim-wave5-execution-brief.md`. T5.1 is now locked as the full SIM connectivity flow: `NeedsSetup` opens a bootstrap modal, setup reuses the host-driven onboarding coordinator with `host = SIM_CONNECTIVITY` as a full-screen SIM overlay, setup success enters the SIM connectivity manager, and later configured entry reopens the manager directly.
  - [x] **2026-03-20 Implementation Note**
...
- [x] **T5.3: Code / Connectivity Isolation**
  ...
  - [x] **2026-03-21 Implementation Note**
    `DiscoveredBadge` is now plain domain data, `RealPairingService` keeps discovered `BlePeripheral` state internal and snapshot-scoped, and `PairingFlowViewModel` owns the shared pairing seam used by both onboarding hosts.
  - [x] **2026-03-21 Focused L1**
    Focused verification is green for `:app-core:compileDebugKotlin` and the Wave 5 unit pack covering `RealPairingServiceTest`, `PairingFlowViewModelTest`, `OnboardingFlowTransitionTest`, `SimConnectivityPairingFlowTest`, and `SimConnectivityRoutingTest`.
- [x] **T5.4: Validation**
...
- [x] **T12.1: Scheduler-Drawer Voice Reschedule Runtime**
  ...
  - [x] this historical Wave 12 note is superseded by the later shared contract tightening: scheduler reschedule now requires an explicit target plus a new exact time in the same utterance, while omitted-target phrases such as `改到3点` and delta-only phrases such as `推迟1小时` / `提前半小时` safe-fail without mutation
...
- [x] **T12.2: Focused Verification**
  ...
  - [x] **2026-04-03 REC Failure Copy Hardening**
    The scheduler-local `REC` lane now normalizes terminal create-family reject text before surfacing `pipelineStatus` / `conflictWarning`, so the visible yellow warning banner stays on scheduler-owned product copy rather than leaking raw classifier/extractor wording. Scheduler-owned reschedule unsupported copy remains unchanged.
