...
export const WelcomeStep: React.FC<{ onNext: () => void }> = ({ onNext }) => (
    ...
    >
        ...
        <h1 className="text-3xl font-bold text-prism-primary mb-3 tracking-tight">SmartSales</h1>
        <p className="text-prism-secondary mb-12 text-lg leading-relaxed">您的 AI 销售教练</p>
        <button onClick={onNext} className="w-full py-4 bg-prism-primary text-prism-bg rounded-2xl font-bold shadow-lg shadow-white/10 active:scale-95 transition-all flex items-center justify-center gap-2 group">
    ...
    >
        <div className="glass-card p-6 mb-4 border border-white/10 bg-white/5 backdrop-blur-xl rounded-3xl">
            <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-full bg-prism-accent/20 text-prism-accent flex items-center justify-center"><Mic size={24} /></div>
                <div className="text-left">
                    <h3 className="font-bold text-prism-primary">麦克风权限</h3>
                    <p className="text-xs text-prism-secondary">用于实时对话分析</p>
                </div>
                <button onClick={onNext} className="ml-auto px-4 py-2 bg-prism-accent hover:bg-prism-accent/80 text-white text-sm font-medium rounded-full transition-colors">允许</button>
            </div>
        </div>
        <div className="glass-card p-6 border border-white/10 bg-white/5 backdrop-blur-xl rounded-3xl">
             <div className="flex items-center gap-4 mb-4">
                ...
                <div className="text-left">
                    <h3 className="font-bold text-prism-primary">蓝牙权限</h3>
                    <p className="text-xs text-prism-secondary">连接 SmartBadge</p>
                </div>
...
export const HandshakeStep: React.FC<{ onNext: () => void }> = ({ onNext }) => {
    ...
    return (
        ...
        >
            <h2 className="text-xl font-bold text-prism-primary mb-8">让我们先认识一下</h2>
            <p className="text-prism-secondary mb-12">试着说：“你好，帮我搞定这个客户”</p>
            ...
            <p className="text-xs text-prism-secondary/70 font-mono">正在聆听...</p>
        </motion.div>
    );
};
