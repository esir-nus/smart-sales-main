...
- [ ] Read `docs/specs/Architecture.md` to enforce the Data-Oriented OS contract
...
- **Mandatory Mechanical Verification:** The plan MUST include a step to execute a mechanical test script (e.g., executing a `grep` script for forbidden imports, or extracting the schema via `kotlinx.serialization` or Reflection) to mathematically validate that the code aligns with the Contract. Do not rely on "vibe checking" the interface map. If the plan relies on "reading the code to check alignment", REJECT IT.
