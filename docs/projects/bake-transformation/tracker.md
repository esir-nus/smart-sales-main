...
| 04-b | connectivity-intent-driven-reconnect-modal-law | in-progress | Landed the reconnect-owner/prompt/card/prompt-lifetime code delta with focused and full `:app-core:testDebugUnitTest` green; installed-debug `platform-runtime/L2.5` loops A and C now pass, while loop B, loop D, and physical L3 closure remain pending. | [sprints/04-b-connectivity-intent-driven-reconnect-modal-law.md](sprints/04-b-connectivity-intent-driven-reconnect-modal-law.md) |
...
- Connectivity reconnect law now follows updated user-intent ownership:
  ...
  disconnected-first active cards, and prompt-lifetime clearing. Installed-debug
  `platform-runtime/L2.5` evidence now covers the latest-intended-active-present
  branch and the manual-default-suppression branch, while the
  latest-intended-active-absent chooser branch, stale mismatch-clearing branch,
  and physical L3 closure remain pending.
