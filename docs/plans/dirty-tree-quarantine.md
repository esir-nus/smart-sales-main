...
- A DTQ-04 follow-up pass on 2026-04-15 repaired the current worktree's compile drift without widening scope ownership: runtime shell now compiles against the shell-side auto-reconnect and sync-island compat seams again, SIM audio drawer/viewmodel regained the expected last-sync and island-event support, and session preview audio-context derivation is back under the shell-owned compatibility layer. Focused verification passed with `./gradlew --offline :app-core:compileFullDebugKotlin`.
