...
### 2026-04-21
- **docs / governance**: Restored the Codex [`cnp` skill](../../.codex/skills/cnp/SKILL.md) on `develop` from the governance branch so the daily trunk again exposes the save-point workflow used to stage, commit, and push the current branch without switching branches, merging, or opening PRs. The restored skill keeps the existing protected-branch warnings for `develop`, `platform/harmony`, and `master`. Verification: compared the restored file against `codex/safety-before-governance-cleanup:.codex/skills/cnp/SKILL.md`; no runtime tests needed for this docs/skill sync.
- **android / scheduler + connectivity**: Badge BLE task-create signal FIXED. Successful scheduler task creation now emits the same best-effort BLE payload `commandend#1` through a pure `TaskCreationBadgeSignal` seam bound to Android `DeviceConnectionManager.notifyTaskCreated()`, while the previously shipped reminder-alarm path through `TaskReminderReceiver -> notifyTaskFired()` remains unchanged. The create-success emit was wired into `IntentOrchestrator`, `SimSchedulerMutationCoordinator`, and onboarding quick-start commit so Android now follows the intended dual-trigger model: create success + reminder fire. Verification: targeted unit coverage for `DefaultDeviceConnectionManagerIngressTest`, `IntentOrchestratorTest`, `SimSchedulerMutationCoordinatorTest`, and `OnboardingSchedulerQuickStartCommitterTest`.
- **tools / harmony**: Added [`scripts/harmony-lane-proof.sh`](../../scripts/harmony-lane-proof.sh) to resolve the newest signed HAP for a declared Harmony lane and print the matching `hdc install`, `aa start`, and `hilog` commands. The helper now fails closed for lanes without a matching signed artifact, which prevents `com.smartsales.harmony.app` from accidentally borrowing `smartsales.HOS.test` proof. Verification: `scripts/harmony-lane-proof.sh hs-test --path-only`; `scripts/harmony-lane-proof.sh app` (expected fail-closed); `scripts/harmony-lane-proof.sh hui` (expected fail-closed because root absent).
- **docs / harmony**: Streamlined the Harmony signed-HAP device-proof workflow. [`docs/platforms/harmony/test-signing-ledger.md`](../platforms/harmony/test-signing-ledger.md) now defines a paved-road lane contract tying compile, signed HAP, bundle ID, `hdc install`, launch, and `hilog` evidence to the same Harmony app identity; [`docs/sops/harmony-operator-runbook.md`](../sops/harmony-operator-runbook.md) now requires operators to use that same contract before advancing a slice; [`docs/projects/harmony-native/tracker.md`](./harmony-tracker.md) now explicitly forbids transferring `smartsales.HOS.test` proof to `com.smartsales.harmony.app`. Verification: doc review against `.agent/rules/lessons-learned.md` and `docs/reference/agent-lessons-details.md` section `Harmony Build Proof Is Not Device Proof`.

### 2026-04-20
- **android / connectivity**: Badge control signals SHIPPED. `TaskReminderReceiver` now emits one best-effort badge chime through `DeviceConnectionManager.notifyTaskFired()`, the BLE gateway stack adds `sendBadgeSignal(...)`, and both `UserCenter` and `ConnectivityModal` now expose persisted badge voice-volume controls that send one best-effort BLE `volume#<0..100>` command only on release while de-duping only after a confirmed prior badge write. Verification: `./gradlew :app:assembleDebug`; `./gradlew :app-core:testFullDebugUnitTest --tests com.smartsales.prism.data.connectivity.legacy.DefaultDeviceConnectionManagerIngressTest --tests com.smartsales.prism.ui.components.connectivity.ConnectivityViewModelTest --tests com.smartsales.prism.ui.settings.UserCenterViewModelTest`. Branch: `develop` ship target from reconstructed `android` lane. Ignored dirt: source worktree `harmony/scheduler-phase-2b` still contained unrelated mixed Android/Harmony/docs changes; only the reconstructed badge-control scope was landed here.
- **docs / connectivity**: Synced badge-control contracts for `UserCenter`, reminder notifications, the interface map, and the tracker so `develop` docs now describe the shipped badge chime and persisted voice-volume behavior. Verification: doc sync reviewed against the Android lane plus the same `assembleDebug` and targeted `testFullDebugUnitTest` reruns. Branch: `develop` ship target from auto-split `docs` lane. Ignored dirt: source worktree `harmony/scheduler-phase-2b` still contained unrelated mixed Android/Harmony/docs changes outside this badge-control sync.
...
- **scheduler**: Wave 13 Scheduler Domain Isolation SHIPPED. Executed "Nuke and Pave" Phase 3. The `TimelineItemModel` has been fully removed from all `domain` interfaces, `data` repositories, testing fakes, and integration testing suites, replacing it with pristine Kotlin domain data classes (`ScheduledTask` and `SchedulerTimelineItem`). The domain is completely decoupled from UI properties (such as `tipsLoading`) and verified natively via the `Acceptance Team` mechanical test suite.
