...
- **[发布] HarmonyOS 日程 Phase 2B 交付** — 完成 HS-006 Phase 2B，补齐 HarmonyOS 日程链路的本轮交付范围。
- **[新增] 徽章设备控制信号** — 新增设备控制信号通道，可承载徽章侧音量等快速控制指令。
- **[新增] 智能对话气泡支持长按复制** — 聊天气泡新增长按复制能力，便于转存 Agent 输出内容。
...
- **[新增] 连接后隔离状态提示** — 连接建立后会显式暴露隔离态与锁存结果，便于识别连接成功但链路不可用的异常场景。
- **[新增] WiFi 修复切换为 transport-first 流程** — 徽章同步异常时优先按传输链路状态推进 WiFi 修复，减少盲目重试。
- **[修复] 配对后疑似 WiFi 隔离识别** — 配对完成后若出现疑似 WiFi 隔离，系统会更早标记异常并引导修复。
- **[新增] HarmonyOS Tingwu 音频链路加固** — 对 HS-010 音频管线进行加固，降低 HarmonyOS 端 Tingwu 处理不稳定问题。
- **[修复] HarmonyOS Tingwu L3 收尾缺口** — 补齐 HS-010 L3 遗留问题，减少 HarmonyOS 音频结果偏差与后续返工。

### 2026-04-17

- **[新增] 同步失败时提示 WiFi 修复** — 当同步失败与链路异常相关时，界面会直接给出 WiFi 修复引导。
- **[修复] 前台服务启动生命周期保持** — 修复前台启动时的生命周期保持问题，降低服务被系统提前回收的风险。
- **[修复] 日程显式 delta 重排支持** — 日程调度链路恢复对显式 delta reschedule 的支持，避免变更未正确落地。
- **[重构] 移除废弃 Harmony flavor** — 清理已废弃的 Harmony flavor 配置，减少双平台演进时的旧分支负担。
...
- **[修复] HarmonyOS Tingwu 说话人区分与详情展示对齐** — 调整 Tingwu diarization 与详情 UI 映射，减少人物区分和结果展示错位。
