...
7. Structural blockers and anti-god-file work belong in `docs/projects/god-file-cleanup/tracker.md`; only the UI-facing dependency summary belongs here.
...
| UI Trunk Structure Cleanup | Refactor-first cleanup of shared UI and ViewModel trunk files that are slowing approved prototype transplants | `docs/projects/god-file-cleanup/tracker.md` | Transplanting | Continue active god-file cleanup waves outside this tracker | This is a UI-facing dependency record, not the owning structure tracker. Approved now: trunk cleanup is a prerequisite for faster future transplants. Deferred here: detailed structural wave law stays in `god-tracker.md`. |
