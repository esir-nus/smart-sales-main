...
The `LightningRouter` acts as the human-in-the-loop gatekeeper for the entire Dual-Engine architecture. It sits at **Phase 0** within the `PrismOrchestrator`, determining whether user input should be handled by the ephemeral **Mascot (System I)** or routed to the heavy-duty **AnalystPipeline (System II)**. 
...
4. **`DEEP_ANALYSIS` / `CRM_TASK`**: Requires multi-step reasoning, schedule modification, or deep investigation.
   - *Routing*: Passed directly to the full `AnalystPipeline` (System II) without further inspection by this router.
...
|------|----------|-------------|
| **Gateway Routing** | PrismOrchestrator| If `query_quality` is NOISE or GREETINGS, `AnalystPipeline` is never called. |
...
| **5** | **Architecture Nuke & Pave** | 🚧 IN PROGRESS | Move Router to the top of `PrismOrchestrator`, severing ties from `AnalystPipeline`. Rename interfaces. |
