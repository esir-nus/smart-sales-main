...
| `DTQ-06` | Governance, trackers, and repo guardrails | `.gitignore`, `.github/**`, `docs/specs/platform-governance.md`, `docs/reference/platform-targets.md`, `docs/platforms/**`, `docs/plans/tracker.md`, `docs/plans/sim-tracker.md`, `docs/cerb/interface-map.md`, dirty-tree audit/control-plane docs | feature-owned code lanes except for final docs sync references | `Accepted` | `Aligned` | docs consistency re-read plus local guardrail checks mirroring `.github/workflows/platform-governance-check.yml` | — | Accepted on 2026-04-04 after docs re-read and local guardrail verification; reopen only if control-plane files change again. |
