...
- **[新增] HarmonyOS 日程 Phase 2C 持久化流程** — HarmonyOS 平台 lane 补齐原生日程仓储、任务持久化、重排期与冲突处理流程，并同步更新入口与 hvigor 配置。
- **[维护] HarmonyOS 音频与 Tingwu 存储链路补强** — 补强 HarmonyOS 文件存储、OSS、Tingwu 服务与运行时配置接线，同时加入 lane proof 脚本和签名台账更新，便于继续独立推进 `platform/harmony`。
