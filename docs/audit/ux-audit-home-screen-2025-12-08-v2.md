...
**OK – Hamburger icon** – `HomeScreen.kt:827-832` – Hamburger icon correctly opens history drawer with `HISTORY_TOGGLE` tag.

**MISMATCH – Missing session title in top bar** – `HomeScreen.kt:811-814` – The top bar does not display the session title at all. It only shows "AI 助手" on the left side, with icons on the right. The contract requires the session title to appear between hamburger and device indicator.

**OK – Device connection indicator** – `HomeScreen.kt:816-825` – Device connection indicator correctly shown when connected.
...
**MISMATCH – Missing "+" new chat button** – `HomeScreen.kt:796-860` – The top bar does not include a "+" new chat button with tag `home_new_chat_button`. The `HomeTopBar` composable only shows hamburger, title, device indicator (if connected), HUD dot (if enabled), and profile icon. The "+" button should appear between HUD dot and profile icon.

**OK – Profile icon** – `HomeScreen.kt:852-857` – Profile icon present with correct tag.

### 2. Top bar title source

**MISMATCH – Title not bound to session** – `HomeScreen.kt:811-814` – Title is hard-coded as "AI 助手" instead of using `state.currentSession.title`. The `HomeTopBar` composable does not receive the current session title as a parameter, and the call site at `HomeScreen.kt:406-416` does not pass it.
...
1. **Top bar still showing "AI 助手" instead of session title**
   ...
   - **Additional issue:** The top bar layout does not include a separate session title element between hamburger and device indicator. The contract requires the title to be a distinct element, not just replacing "AI 助手".

2. **Missing "+" new chat button in top bar**
   - **Cause:** `HomeTopBar` composable does not include the new chat button in its layout.
   - **Location:** `HomeScreen.kt:796-860` – `HomeTopBar` function definition and call site at `HomeScreen.kt:406-416`.
   - **Additional issue:** The call site does not pass `onNewChatClicked` callback to `HomeTopBar`, even though `onNewChatClicked` is available in the parent scope.

3. **Top bar layout order incorrect**
   - **Cause:** Current implementation shows hamburger on left, "AI 助手" text, then all icons on right. The contract requires: hamburger → session title → device indicator → HUD dot → "+" button → profile icon, all in a single row.
   - **Location:** `HomeScreen.kt:804-859` – The `Row` layout groups icons on the right but does not follow the exact left-to-right order specified in the contract.
...
2. Missing session title element in top bar layout (title should appear between hamburger and device indicator).
3. Missing "+" new chat button in top bar.

**Minor issues:**
- Top bar layout order does not exactly match contract specification (icons grouped on right instead of strict left-to-right order).

**Recommendations:**
1. Update `HomeTopBar` to accept `currentSessionTitle: String` parameter and display it instead of "AI 助手".
2. Restructure top bar layout to show elements in exact order: hamburger → session title → device indicator → HUD dot → "+" button → profile icon.
3. Add "+" new chat button to `HomeTopBar` between HUD dot and profile icon, wired to `onNewChatClicked` callback.
4. Verify at runtime that history drawer titles match main header titles.
