...
**Issue**: `SessionMemory` is listed in `xIndex.payloadTypes` array (line 40), but schema marks it as deprecated (line 565: `$comment: "DEPRECATED: use M2BTranscriptionState"`)

**Impact**: Agents may see `SessionMemory` in payloadTypes list and assume it's a current canonical type

**Evidence**:
- Line 40: `"SessionMemory"` appears in `deprecatedPayloadTypes` array (correct)
- Line 24-38: `payloadTypes` array does NOT include `SessionMemory` (correct)
...
**Note**: Upon closer inspection, `SessionMemory` is correctly placed in `deprecatedPayloadTypes` (line 40), not in `payloadTypes`. However, it remains in `SinglePayload.oneOf` (line 67) for backward compatibility, which is acceptable.
...
- `docs/api-contracts.md` §2.2 (line 74): References extraction algorithm in §5.2
- `docs/api-contracts.md` §5.2 (lines 131-135): Detailed extraction algorithm matches spec
...
- `docs/api-contracts.md` §5.2 (lines 131-135): Matches extraction algorithm (canonical location)
- `docs/api-contracts.md` §2.2 (line 74): Short reference to §5.2 (no duplication)
...
1. **P1-2**: `PublishedAnalysis.description` references deprecated `SessionMemory` (should reference `M2BTranscriptionState`)
...
- `docs/orchestrator-v1.schema.json` line 786: `PublishedAnalysis.description` references `SessionMemory`
