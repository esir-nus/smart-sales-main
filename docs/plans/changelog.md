...
- **docs / harmony**: Streamlined the Harmony signed-HAP device-proof workflow. [`docs/platforms/harmony/test-signing-ledger.md`](../platforms/harmony/test-signing-ledger.md) now defines a paved-road lane contract tying compile, signed HAP, bundle ID, `hdc install`, launch, and `hilog` evidence to the same Harmony app identity; [`docs/sops/harmony-operator-runbook.md`](../sops/harmony-operator-runbook.md) now requires operators to use that same contract before advancing a slice; [`docs/plans/harmony-tracker.md`](./harmony-tracker.md) now explicitly forbids transferring `smartsales.HOS.test` proof to `com.smartsales.harmony.app`. Verification: doc review against `.agent/rules/lessons-learned.md` and `docs/reference/agent-lessons-details.md` section `Harmony Build Proof Is Not Device Proof`.
