// File: feature/chat/src/main/java/com/smartsales/domain/transcription/SanitizerImpl.kt
// Module: :feature:chat
// Summary: Transcription display cleanup implementation per Orchestrator-V1 Section 3.2 (no semantic changes)
...
/**
 * SanitizerImpl: transcription display cleanup implementation.
 */
@Singleton
class SanitizerImpl @Inject constructor() : Sanitizer {

    private val timeFormatter = DecimalFormat("00")

    override fun buildMarkdown(
    ...
    override fun buildDiarizedSegments(transcription: TingwuTranscription?): List<DiarizedSegment> {
    ...
    override fun buildSpeakerLabels(
        transcription: TingwuTranscription?,
    ...
    private fun shouldMergeAsSubtitle(previous: DiarizedSegment, next: DiarizedSegment): Boolean {
        ...
        val combinedDuration = max(next.endMs, previous.endMs) - kotlin.math.min(previous.startMs, next.startMs)
